...
class ProperAH_UGC:
    """
    Proper AH-UGC implementation following the paper methodology with memory optimizations
    """

    def __init__(self, num_projectors: int = 8, p_stable_alpha: float = 1.0, chunk_size: int = 1000):
        """
        Initialize AH-UGC framework
    ...
    def create_augmented_features(self, features: torch.Tensor,
        ...
        adj_feature = adj_normalized.unsqueeze(1)

        augmented = torch.cat([
            (1 - alpha) * features,
            alpha * adj_feature.expand(-1, features.size(1))
        ], dim=1)
    ...
    def compute_lsh_hash_scores(self, augmented_features: torch.Tensor, node_type: str) -> torch.Tensor:
        ...
        for start_idx in range(0, num_nodes, self.chunk_size):
            ...
            projections = torch.matmul(feature_chunk, W) + b
...
def run_gnn_experiments(original_data, coarsened_data, target_node_type,
    ...
    if not GNN_MODELS_AVAILABLE:
        print("Skipping GNN experiments (models not available)")
        return {}
    ...
    print(f"Coarsened data: {coarsened_train_mask.sum()} train, {coarsened_val_mask.sum()} val, {coarsened_test_mask.sum()} test")
...
def run_ah_ugc_experiment(data_path, dataset_name, chunk_size=500):
    ...
    if missing_types:
        print(f"Warning: Missing node types in dataset: {missing_types}")
    ...
    if target_node_type not in available_types:
        print(f"Warning: Target node type '{target_node_type}' not in dataset")
...
def run_multi_dataset_experiments(data_path, datasets, chunk_size=500):
    ...
    if all_results:
        ...
        try:
            ...
            print(f"\nCould not save combined results: {e}")
...
def main():
    ...
    parser.add_argument('--epochs', type=int, default=200,
                        help='Number of training epochs for GNN models (default: 200)')
    ...
    if len(args.datasets) == 1:
        ...
        print(f"\nRunning single dataset experiment: {dataset_name.upper()}")
        ...
        print(f"\nRunning multi-dataset experiments: {[d.upper() for d in args.datasets]}")
